...
- 内部后台接口要求已登录用户。
- 客户确认页接口不要求 Supabase 登录，但必须校验安全 token、手机号验证码和访问状态。
...
- status
- minRankId
- scope：当前支持 `assigned`、`sourced`、`mine`，律师可查询自己承办、自己发起或两者合并的任务。
- sort：`createdAtDesc`、`createdAtAsc`、`dueAtAsc`、`amountDesc`、`statusAsc`

角色差异：

- 律师：看开放任务、自己发起的任务和自己承办的任务。
- 财务：看与结算相关的任务摘要。
- 主任：看本组织任务。

### POST /api/tasks

创建任务，并自动生成客户确认页安全 token。
...
- 任务必须是 `open`。
- 律师职级必须满足任务最低职级。
- 当前律师已承办任务上不能存在未办结的三级严重或四级重大风控工单；否则返回 `409 CONFLICT`，提示先处理风控工单。
...
默认临时验证码为 `111111`。如果本组织在系统参数中保存了 `customer_portal_demo_code`，验证码校验会读取该参数值。
...
- token 必须存在且 active。
- 必须先通过验证码校验。
- 如果任务已由发起人验收，响应中的交付附件会包含客户侧下载地址；任务未验收时只返回附件元数据，不开放下载。
...
当前最多导出 1000 条，按生成时间倒序排列。导出字段包括结算 ID、任务、承办律师、用户名、职级、任务金额、结算比例、原结算金额、扣减金额、律师实付金额、扣罚去向、扣罚锁定时间、状态、生成时间和确认时间。金额按元保留两位小数，便于财务在表格中汇总。
...
权限：系统管理员、律所管理员、主任、发起人、承办律师。

角色差异：

- 承办律师：只能查看自己承办任务关联的风控工单，并用于提交 48 小时答辩。
- 发起人：只能查看自己登记的风控工单。
...
- 工单已有委员会裁决时不能重复提交。
- 已办结工单不能提交委员会裁决。
- 承办律师已提交答辩，或 48 小时答辩期已到期后，才允许提交裁决。
...
- `customer_portal_demo_code`：客户访问验证码。
...
- `demo_mode_hint_enabled`：是否显示运行模式提示。
...
- `customer_portal_demo_code` 会影响客户确认页验证码校验。
