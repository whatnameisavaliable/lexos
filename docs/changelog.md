...
- 客户大屏 demo/API 统一验证码提示、交付附件、下载入口、确认评分按钮和错误反馈样式。
...
- 统计口径按办案律师归集，排除委员会裁决为“无过错”的工单；高/重大风控、警示、扣减和升级处理会进入有效风控口径。
- 总览页新增“累犯惩戒建议”运营信号和建议面板，展示观察周期、需处理律师、限制建议、升级复盘、最近工单和建议动作。
- “投诉与风控”页新增“累犯建议”指标，并复用同一建议面板；办案律师视角只展示本人结果，管理/主任视角展示全体办案律师汇总。
...
### 风控：抢单权限限制第一版
- 新增 `src/lib/tasks/claim-restrictions.ts`，集中维护抢单风控限制规则：未办结三级严重或四级重大风控工单会暂停办案律师抢新任务，中低风险和已办结工单不影响抢单。
- `POST /api/tasks/:id/claim` 在原有开放状态、办案律师身份和最低职级校验后，增加当前律师已承办任务的高/重大未结风控校验；被限制时返回 `409 CONFLICT`。
- Demo 任务大厅接入同一规则，顶部展示风控暂停提示，按钮显示“风控暂停”并禁用；标准演示数据中 `lawyer01` 可演示被严重未结风控拦截，`lawyer02` 的中等风控不拦截。
- 新增 `tests/claim-restrictions.test.ts` 并纳入 `npm test`，覆盖阻断规则、非阻断规则、统一抢单判断和 Demo 样例。
...
- 新增 `GET /api/funds`，系统管理员、律所管理员和财务可读取本组织四类账户摘要和分页资金流水；办案律师不可访问资金台账。
...
- 总览统计、客户渠道统计、办案律师绩效和个人工作台改用扣后实付金额作为有效结算口径。
...
- 新增 `POST /api/risk-cases/:id/decision`，系统管理员、律所管理员和主任可在办案律师已答辩或 48 小时答辩期结束后提交裁决，提交会写入 `risk_cases.committee_decide` 审计日志。
...
- `GET /api/risk-cases` 新增办案律师读取范围：只返回本人承办任务关联的风控工单。
- 新增 `POST /api/risk-cases/:id/defense`，办案律师可在工单创建后 48 小时内提交本人任务答辩，提交会写入 `risk_cases.submit_defense` 审计日志。
- 后台“风控”菜单对办案律师开放；风控页新增“48 小时答辩”列，展示截止时间、剩余小时、答辩内容和提交入口。
- Demo 数据新增一条办案律师可答辩样例，E2E 冒烟流程已覆盖办案律师提交答辩。
...
- 新增 `PATCH /api/risk-cases/:id`，按组织、角色和案源律师登记人边界更新工单状态，并写入 `risk_cases.update_status` 审计日志。
...
- 后台新增“风控”菜单和“投诉与风控”页面，系统管理员、律所管理员、主任和案源律师可查看风险指标、登记工单并筛选列表。
...
- 客户侧安全访问本期继续使用现有 token + 手机号 + 演示验证码机制，不接入真实短信 MFA 或短信服务商。
...
- 办案律师绩效统计新增 `rollingAverageScore` 和 `rollingTaskCount`，基于现有客户反馈、案源验收评分和案件结果评分计算近期交付质量。
- “近 30 单”按评分相关时间倒序截取每名办案律师最近最多 30 个有评分任务；同一任务可聚合客户评分、案源评分和案件结果评分。
- 总览页“办案律师绩效”表格新增“近30单”列，右侧运营信号新增“近30单评分”，方便管理层快速观察近期质量。
...
### 协作：审核律师流程第一版
- 新增 `src/lib/tasks/review.ts`，集中维护审核结论校验、可审核角色、审核权限判断和案源验收前置判断。
...
- 新增 `POST /api/tasks/:id/review`，主任、律所管理员和系统管理员可以审核通过或退回修改；退回后任务回到办案律师可重新提交状态。
- `POST /api/tasks/:id/approve` 增加审核前置保护：需要复核的任务必须审核通过后，案源律师才能最终验收与评分。
- “我的任务”允许主任和律所管理员进入；任务行新增审核状态徽标、审核意见表单、审核通过/退回修改按钮，以及详情中的审核状态、审核时间和审核意见。
- 任务里程碑中的“案源验收”升级为“审核与案源验收”，可展示待审核、退回修改、待案源验收和已验收状态。
- 标准演示数据新增待审核任务和审核通过历史样例，方便主任/案源律师视角演示。
...
### 运营：案源律师评分 / 案件结果评分第一版
- 新增 `src/lib/reviews/source-review.ts`，统一校验案源验收评分、案件结果评分、案源评语和结果摘要；评分限定为 1 到 10 的整数，文本字段会清理空白并限制长度。
- 新增 Supabase 迁移 `20260609042505_add_task_source_review_scores.sql`，为 `tasks` 增加案源评分、案源评语、案源评分时间、案件结果评分和结果摘要字段，并增加评分范围约束。
- `POST /api/tasks/:id/approve` 支持在案源律师验收时一并保存评分和评价摘要，并把评分写入审计 metadata；任务列表 API 与前端 API client 已补充新字段映射。
- “我的任务”中的验收动作升级为评分表单，案源律师可在确认验收前填写案源评分、案件结果评分、案源评语和结果摘要；任务详情会展示已采集评分。
- 办案律师绩效统计新增案源平均评分和案件结果平均评分；总览页表格和运营信号同步展示，律师个人工作台的“综合评分”纳入客户评分、案源评分和结果评分。
...
- 新增角色菜单权限单元测试，覆盖默认入口、办案律师边界、律所管理员权限、客户/渠道商不能进入内部后台和菜单 key 唯一性。
...
- 客户大屏 API 映射交付附件元数据；已验收任务显示下载入口，未验收任务显示“案源验收后开放”。
- 内存 Demo 客户大屏同步展示模拟附件，保持真实 API 模式和 Demo 模式体验一致。
- 验证通过：`npm run lint`、`npm run typecheck`、`npm test`、`npm run build`、沙箱外显式绕过本机代理执行 `npm run test:e2e`。
...
- 新增 `POST /api/tasks/:id/deliverables`，办案律师可提交带附件的成果；服务端校验任务归属和状态后上传私有 Storage、写入交付记录并推进任务状态。
- 新增 `GET /api/tasks/:id/deliverables/:deliverableId/download`，系统管理员、律所管理员、任务案源律师和任务办案律师可通过 5 分钟 signed URL 下载内部交付附件。
...
- 新增 `src/lib/tasks/progress.ts`，统一把任务状态转换为发布、接单、成果提交、案源验收、客户确认和结算记录里程碑。
...
- 新增 `src/lib/analytics/lawyer-performance.ts`，基于现有用户、任务、客户反馈和结算数据聚合办案律师绩效。
- 总览页新增“办案律师绩效”面板，展示律师、职级、在办/完成任务、客户评分、任务金额和结算金额。
- 右侧信号栏新增绩效领先律师、有效办案律师、客户平均评分、在办任务、已完成任务和已确认结算。
...
- 客户大屏验证码校验接入 `customer_portal_demo_code`，默认仍为 `111111`，真实 API 模式可通过“参数”页修改。
...
- 新增单元测试覆盖系统参数默认分页、客户大屏验证码覆盖和结算锁定期边界。
...
- 结算导出权限沿用结算列表：管理员、律所管理员和财务可导出本组织结算；办案律师只能导出自己的结算。
...
- 新增 `tests/e2e/lexos-preview-smoke.spec.ts`，覆盖 `/api/health`、登录页、内存 demo 管理员首次改密、总览页、客户大屏入口和结算页。
...
- 客户大屏校验使用 `LEXOS-DEMO-004 / 13800000000 / 111111`，不点击“确认接收并评分”，避免改变标准演示数据。
...
- 扩充内存 demo 标准数据：9 个用户、4 个客户、9 个任务，覆盖待抢单、处理中、待验收、客户待确认、待结算、已结算和已取消状态。
...
- 登录页固定展示核心演示账号，避免因为数据顺序变化漏掉办案律师和财务账号。
- 新增 `docs/demo-guide.md`，整理演示目标、推荐账号、标准脚本、客户大屏演示、风险处理和进入可运营阶段检查清单。
...
- 我的任务列表展开区新增任务时间线，展示发布任务、办案接单、成果提交、案源验收、客户确认和结算记录状态。
...
- 后台新增“参数”导航和系统参数页面，可维护客户大屏演示验证码、结算锁定天数、默认分页数量和演示模式提示开关。
...
- `/api/tasks` 支持服务端分页、状态筛选和搜索，同时保留案源律师、办案律师、财务和管理员的可见性规则。
- `/api/settlements` 支持服务端分页、状态筛选和搜索，同时保留办案律师只能看自己结算的规则。
...
- 后台新增全局成功提示，创建用户、创建客户、发布任务、抢单、提交成果、任务验收、客户确认和财务结算等关键操作完成后会给出明确反馈。
- 关键操作新增 loading 状态，避免重复点击；任务验收和财务确认结算新增内联二次确认。
- 我的任务列表新增任务详情展开区，展示任务流程节点、客户、办案律师、任务类型、金额、职级、截止日期、客户 token、验收时间和成果链接。
...
- 已记录登录成功、修改密码、创建用户、创建客户、创建任务、抢单、提交成果、任务验收、客户确认评分和财务确认结算。
- 审计日志 metadata 不记录密码和客户大屏 token 明文。
...
- 真实闭环已跑通：创建客户、发布任务、抢单、提交成果、案源律师验收、客户验证码确认、评分、生成结算、财务确认结算。
...
- API 模式已接入登录、首次改密、用户列表/创建、职级、客户列表/创建、任务列表/创建/抢单/提交/验收、客户大屏验证码/评分、结算列表/确认。
...
- 新增创建用户输入校验：默认密码 `111111`、首次登录强制改密、办案律师必须绑定职级。
- 新增服务端 Supabase admin client，service role 仅在服务端使用。
- 新增内部会话和角色校验 helper。
- 新增 API：用户名登录、修改密码、用户列表、创建用户、职级列表。
- 新增 API：客户列表、创建客户。
- 新增 API：任务列表、创建任务、抢单、提交成果、案源律师验收。
- 新增 API：客户大屏验证码校验、客户大屏数据、客户确认评分。
- 新增 API：结算列表、财务确认结算。
- 新增 token hash 工具，客户大屏 token 明文不入库。
...
- 实现办案律师任务大厅抢单和成果提交。
- 实现案源律师验收任务。
- 实现客户大屏 demo：任务 token + 手机号 + 固定验证码 `111111`。
- 实现客户确认接收、评分、自动生成待结算记录。
- 实现财务确认结算。
- 新增核心业务规则测试：结算金额、强制改密、抢单权限、任务状态流转、客户验证码。
