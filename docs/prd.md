...
| 域 | 范围摘要 |
|----|----------|
| 用户与权限 | RBAC、账户启停、内置管理员初始化、个人中心、虚拟邮箱认证、高权限 MFA |
...
| `admin` | 系统管理员 | 启用 | 全量系统配置、用户管理、审计、AI 配置；**强制 MFA** |
| `lawyer` | 律师 | 启用 | 转写任务、个人云盘、全文检索（仅本人数据） |
| `director` | 律所主任 | **预留** | 首期无业务模块权限；后续补充。账户若启用则 **强制 MFA** |
| `client` | 客户 | **预留** | 首期无业务模块权限；后续补充 |
| `channel` | 外部渠道商 | **预留** | 首期无业务模块权限；后续补充 |
...
| 重置他人密码为初始密码 | U | — | — | — | — | — |
| **个人中心** | | | | | | |
| 查看本人资料 | R | R | R【待确认】 | R【待确认】 | R【待确认】 | — |
| 修改本人密码（需原密码） | U | U | U【待确认】 | U【待确认】 | U【待确认】 | — |
| MFA 绑定/管理（TOTP） | U | — | U【待确认】 | — | — | — |
...
| 删除转写任务/结果 | D | D\* | — | — | — | — |
| **个人云盘 / 全文检索** | | | | | | |
| 目录/标签/文件管理 | CUD 全部 | CUD\* | — | — | — | — |
| 全文检索 | R 全部 | R\* | — | — | — | — |
| 下载文件 | R | R\* | — | — | — | — |
| **审计日志** | R | — | — | — | — | — |
| **系统配置（非 AI）** | CUD | — | — | — | — | — |

**说明：** `director` / `client` / `channel` 首期矩阵均为「—」；角色可在用户管理中分配，但登录后仅展示占位或无业务菜单【待确认】占位页文案。后续版本通过 PRD 修订扩展矩阵，不得破坏律师「仅本人数据」原则（除非主任等角色另有书面确认）。

### 2.3 数据隔离原则（RLS）

- 所有业务表强制 **RLS**；禁止仅依赖前端隐藏。
- **`lawyer`**：`transcription_tasks`、`files` 等核心表仅 `created_by = auth.uid()`（或等价外键）可读写。
- **`admin`**：通过策略允许全量读写（或只读+管理写分离【待确认】审计表是否仅 admin 可读）。
- **预留角色**：首期不授予任何业务行访问策略；与矩阵「—」一致。
...
| `SELECT`（读/下载） | 仅当 `auth.uid() = owner_id` 时允许；`admin` 通过独立策略或 service role 管理路径【待确认】管理端直读方式 |
| `DELETE` | 同上；律师仅可删本人对象 |
| `INSERT` / `UPDATE` | 上传路径须使 `owner_id` 与当前用户一致；禁止客户端任意改写他人路径 |
| 签名 URL | 短期签名链接的签发须在后端校验归属后再生成【待确认】有效期 |
...
> **实施说明（2026-05）**：当前开发阶段已**关闭** MFA 与图形验证码；以下为目标态需求，恢复时见 `tasks.md` 后续里程碑。

| 项 | 要求 |
|----|------|
| 适用角色 | **`admin`、`director`**（主任角色启用后同样适用） |
| 机制 | Supabase Auth **TOTP** |
| 绑定时机 | 首次登录或个人中心；展示二维码，支持腾讯身份验证器 / Google Authenticator |
| 登录流程 | 密码验证通过后，须输入 **6 位** TOTP；未绑定且属强制角色则引导绑定后方可进入业务 |

律师、客户、渠道商首期不强制 MFA【待确认】后续是否扩展。
...
| 禁用账户 | 拒绝登录；错误提示不区分「不存在/密码错误」【待确认】是否统一文案 |
...
- **前端路由守卫（Router Guard）**：每次导航前读取 `requires_password_change`（或会话内缓存并经 API 校验）；若为 `true`，**强制重定向**至修改密码页，并：
  - 隐藏/禁用业务侧边栏与业务入口；
  - 拦截对业务 API 的调用（BFF/中间件二次校验【待确认】）。
...
2. 若角色为 `admin`/`director` 且已启用 MFA → 校验 TOTP。
3. 输错密码达 3 次 → 要求验证码；达 5 次 → Auth 层锁定 30 分钟。
4. 若 `requires_password_change = true` 或内置 `admin` 仍为初始密码 → **仅允许**访问改密页，Router Guard 拦截其他路由。
5. 个人中心：展示资料；主动改密须验证原密码；成功后 `requires_password_change = false`。
...
- 密码强度【待确认】最小长度与复杂度。
- 会话策略与 Supabase Session 配置一致【待确认】。
...
| 功能点 KEY | 说明 |
|------------|------|
| `asr_physical` | 语音转文字 - 物理层（声学 ASR） |
| `asr_semantic` | 语音转文字 - 语义层【待确认】与物理层边界 |
| `llm_transcript_polish` | LLM：标点、纠错、口语精简、分段 |
| `llm_legal_summary` | LLM：法律格式摘要 |

**兜底：** 功能绑定模型失败 → 回退全局默认模型，记录降级事件【待确认】重试次数。
...
| MP4 视频 | 庭审常见大体积 MP4 | **禁止浏览器端抽音**（含 ffmpeg.wasm）。流程：TUS **直传 MP4 原文件**至 Storage → 上传完成后由 **Node.js 子进程或异步 Worker** 调用**服务器本地 FFmpeg** 抽音并重采样为 **16 kHz、单声道、16 bit** WAV 或 MP3 → 后续流水线仅使用音频对象；原 MP4 默认删除或【待确认】冷存储 |
...
5. VAD 微服务切片 → 并行 ASR → 合并时间戳；说话人分离【待确认】最大说话人数。
6. LLM 润色与法律摘要 → 工作台音文对照、编辑、导出。
...
| TUS 上传中断 | 断点续传 |
| 抽音失败 | 任务失败；记录原因 |
| VAD/ASR 部分片段失败 | 【待确认】整任务失败 vs 标记缺失时段 |
| LLM 失败 | 兜底模型；仍失败则保留 ASR 稿【待确认】 |
| 律师禁用 mid-task | **不中断**后端流水线：推理队列继续执行并落盘；账号禁用后该律师**无法登录**查看；数据已沉淀，`admin` 可跨人查看/追溯；主任跨人权限待后续角色启用 |

#### 防错机制

- 异步队列；UI 轮询或订阅【待确认】。
- Storage 与 DB 任务状态一致性【待确认】补偿任务。
- 临时对象生命周期与加密【待确认】。
...
stateDiagram-v2
  ...
  AsrRunning --> LlmRunning: ASR 合并成功
  AsrRunning --> Failed: ASR 失败
  LlmRunning --> Completed: LLM 成功
  LlmRunning --> Failed: LLM 与兜底均失败
  Failed --> Queued: 用户重试【待确认】
  Completed --> [*]
```
...
- **默认目录规则（已确认）**：任务完成归档时，系统自动创建层级 **`YYYY-MM-DD/任务名称/`**（任务名称取创建时标题或文件名去扩展名【待确认】截断规则）；律师可后续重命名该目录。
- **禁止**在云盘根目录直接堆砌文件；新建文件必须位于某目录下。
- 管理员可跨用户查看/管理【待确认】是否允许 admin 删除律师文件。
- 目录同级重名、级联删除等【待确认】。
...
- 记录：登录成功/失败、改密、重置密码、上传完成、下载、删除、导出、AI 配置变更等。
- 失败登录【待确认】是否记录尝试用户名。
- 日志保留期【待确认】。
