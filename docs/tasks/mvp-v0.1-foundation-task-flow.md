...
律所希望先用一个 demo 验证内部任务协作是否可行：发起人可以发布任务，承办律师可以承接并提交成果，客户可以通过安全链接查看交付并评分，财务可以看到系统自动生成的结算记录。

## 目标

完成 Lexos 第一条最小业务闭环：

管理员初始化用户和职级 -> 发起人创建客户与任务 -> 承办律师承接 -> 承办律师提交成果 -> 发起人验收 -> 客户通过验证码链接确认接收并评分 -> 系统生成结算记录 -> 财务确认结算。
...
- 发起人：创建客户、创建任务、验收任务、查看自己发布的任务。
- 承办律师：查看符合条件的任务、承接、提交成果、查看自己的结算。
- 财务：查看结算记录、确认结算。
...
2. 管理员登录后创建发起人、承办律师和财务用户。
3. 管理员配置 L1A 至 L3C 的职级和结算比例。
4. 发起人创建客户。
5. 发起人创建任务，填写任务金额、任务说明、截止时间和最低职级条件。
6. 系统为任务生成客户确认页安全链接。
7. 承办律师进入任务大厅，查看符合条件的待抢任务。
8. 承办律师承接后，任务进入处理中。
9. 承办律师提交成果说明或里程碑。
10. 发起人验收任务。
11. 客户打开安全链接，输入验证码，查看任务交付信息。
12. 客户确认接收并提交 10 分制评分。
13. 系统按“任务金额 × 承办律师职级比例”生成待结算记录。
14. 财务确认结算。
15. 系统记录关键审计日志。
...
- 承办律师只能看到可抢任务、自己的任务和自己的结算。
- 发起人只能管理自己创建或被授权的客户、案件和任务。
- 财务只能处理本组织的结算数据。
- 客户确认页不直接开放 Supabase 表访问，由服务端校验安全链接和验证码后返回有限数据。
...
- 客户确认页更强调交付可信度、进度清晰和确认动作。

## 验收标准

- [x] 管理员可以登录系统。
- [x] 管理员可以创建发起人、承办律师和财务用户。
- [x] 管理员可以配置 L1A 至 L3C 职级和比例。
- [x] 发起人可以创建客户和任务。
- [x] 系统可以为任务生成客户安全访问链接。
- [x] 承办律师可以在任务大厅看到符合条件的任务。
- [x] 承办律师可以承接。
- [x] 承办律师可以提交成果说明。
- [x] 发起人可以验收任务。
- [x] 客户可以通过验证码访问客户确认页。
...
- API/集成测试：登录、创建用户、创建任务、承接、提交、验收、客户确认、生成结算。
- Supabase RLS 测试：不同角色只能访问授权范围内的数据。
- UI/浏览器检查：登录、任务大厅、任务详情、客户确认页、结算页主流程。
- 手工验收：用管理员、发起人、承办律师、财务和客户五种身份跑完整闭环。
