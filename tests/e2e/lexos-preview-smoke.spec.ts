...
test.describe("Lexos 远程 Preview 冒烟流程", () => {
  ...
  test("内存工作区可完成登录、客户确认页校验和结算页访问", async ({ page }) => {
    ...
    await page.getByLabel("用户名").fill("director01");
    ...
    await expect(page.getByText("客户确认页")).toBeVisible();
    ...
    await expect(page.getByText("发起人已验收")).toBeVisible();
